...
func (s *Store) GetBoard(projectID string) (*models.Board, error) {
	statuses := []string{"todo", "in_progress", "dev_done", "qa_in_progress", "live_in_progress", "done"}
	board := &models.Board{
		ProjectID: projectID,
...
func (s *Store) ListTodos(section string, completed *bool) ([]models.TodoItem, error) {
	query := "SELECT id, title, description, completed, priority, due_date, section, position, created_at, updated_at FROM todos WHERE 1=1"
	var args []interface{}
	if section != "" {
		query += " AND section = ?"
	...
	for rows.Next() {
		...
		var compInt int
		if err := rows.Scan(
			&t.ID, &t.Title, &t.Description, &compInt, &t.Priority, &t.DueDate,
			&t.Section, &t.Position, &t.CreatedAt, &t.UpdatedAt,
		); err != nil {
			return nil, err
		}
...
func (s *Store) GetTodo(id string) (*models.TodoItem, error) {
	var t models.TodoItem
	var compInt int
	err := s.db.QueryRow(
		"SELECT id, title, description, completed, priority, due_date, section, position, created_at, updated_at FROM todos WHERE id = ?",
		id,
	).Scan(
		&t.ID, &t.Title, &t.Description, &compInt, &t.Priority, &t.DueDate,
		&t.Section, &t.Position, &t.CreatedAt, &t.UpdatedAt,
...
func (s *Store) CreateTodo(req models.CreateTodoRequest) (*models.TodoItem, error) {
	priority := req.Priority
	if priority == "" {
		priority = "low"
	}
	...
	var maxPos float64
	s.db.QueryRow("SELECT COALESCE(MAX(position), 0) + 1000 FROM todos WHERE section = ?", section).Scan(&maxPos)
	...
	_, err := s.db.Exec(
		`INSERT INTO todos (id, title, description, completed, priority, due_date, section, position, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		t.ID, t.Title, t.Description, 0, t.Priority, t.DueDate, t.Section, t.Position, t.CreatedAt, t.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (s *Store) UpdateTodo(id string, req models.UpdateTodoRequest) (*models.TodoItem, error) {
	t, err := s.GetTodo(id)
	if err != nil || t == nil {
		return nil, err
	}
	...
	_, err = s.db.Exec(
		`UPDATE todos SET title=?, description=?, completed=?, priority=?, due_date=?, section=?, position=?, updated_at=? WHERE id=?`,
		t.Title, t.Description, compInt, t.Priority, t.DueDate, t.Section, t.Position, t.UpdatedAt, t.ID,
	)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ToggleTodo(id string) (*models.TodoItem, error) {
	t, err := s.GetTodo(id)
	if err != nil || t == nil {
		return nil, err
	}
	newCompleted := !t.Completed
	return s.UpdateTodo(id, models.UpdateTodoRequest{Completed: &newCompleted})
}
...
func (s *Store) ClearCompletedTodos() error {
