...
func (s *Server) setupRoutes(webFS fs.FS) {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Route("/todos", func(r chi.Router) {
			r.Get("/", s.listTodos)
			r.Post("/", s.createTodo)
			r.Post("/clear-completed", s.clearCompletedTodos)
			r.Get("/{id}", s.getTodo)
			r.Put("/{id}", s.updateTodo)
			r.Patch("/{id}", s.updateTodo)
			r.Post("/{id}/toggle", s.toggleTodo)
			r.Patch("/{id}/toggle", s.toggleTodo)
...
func (s *Server) listTodos(w http.ResponseWriter, r *http.Request) {
	...
	if comp := r.URL.Query().Get("completed"); comp != "" {
		b := comp == "true" || comp == "1"
		completedPtr = &b
	}
	todos, err := s.store.ListTodos(section, completedPtr)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (s *Server) updateTodo(w http.ResponseWriter, r *http.Request) {
	...
	t, err := s.store.UpdateTodo(id, req)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (s *Server) clearCompletedTodos(w http.ResponseWriter, r *http.Request) {
	if err := s.store.ClearCompletedTodos(); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
	w.WriteHeader(http.StatusNoContent)
