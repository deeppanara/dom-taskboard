...
export const api = {
  todos: {
    list: (section?: string, completed?: boolean) => {
      const params = new URLSearchParams();
      if (section) params.set("section", section);
      if (completed !== undefined) params.set("completed", String(completed));
      const q = params.toString();
      return request<TodoItem[]>(`/api/todos${q ? `?${q}` : ""}`);
    },
    get: (id: string) => request<TodoItem>(`/api/todos/${id}`),
    create: (data: Partial<TodoItem>) =>
    ...
    toggle: (id: string) =>
      request<TodoItem>(`/api/todos/${id}/toggle`, {
        method: "POST",
      }),
    delete: (id: string) =>
    ...
    clearCompleted: () =>
      request<void>("/api/todos/clear-completed", {
        method: "POST",
      }),
  },
