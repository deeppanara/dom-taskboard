...
const STATUS_COLORS: Record<string, string> = {
  ...
  dev_done: "bg-purple-500",
  qa_in_progress: "bg-amber-500",
  live_in_progress: "bg-teal-500",
  done: "bg-green-500",
};
...
function TicketCard({
  ...
  return (
    <div
      onClick={onClick}
      className={`rounded-lg border border-slate-200 dark:border-slate-700/50 bg-white dark:bg-slate-900 p-3 space-y-2 transition-all shadow-2xs hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-xs cursor-pointer ${
    ...
    >
      ...
      <p className="text-sm text-slate-800 dark:text-slate-200 leading-snug font-medium">{ticket.title}</p>
      {(project || team) && (
  ...
  return (
    <div className="flex flex-col w-80 shrink-0">
      <div className="flex items-center gap-2 px-1 pb-3">
        ...
        <span className="text-xs text-slate-400 dark:text-slate-600 ml-auto font-medium">{tickets.length}</span>
        <button
          onClick={() => onAddTicket(status)}
          className="text-slate-400 hover:text-slate-700 dark:text-slate-600 dark:hover:text-slate-300 transition-colors p-0.5 rounded hover:bg-slate-200 dark:hover:bg-slate-800"
        >
      ...
      <div
        ref={setNodeRef}
        className={`flex-1 space-y-2 rounded-lg p-2 transition-colors min-h-32 bg-slate-100/50 dark:bg-slate-900/30 border border-slate-200/50 dark:border-slate-800/30 ${
          isOver ? "bg-blue-500/10 dark:bg-blue-500/5 ring-1 ring-blue-500/30 dark:ring-blue-500/20" : ""
        }`}
      >
        {tickets.map((ticket) => (
          <DraggableTicket
        ...
        {tickets.length === 0 && (
          <div className="flex items-center justify-center h-24 text-xs text-slate-400 dark:text-slate-700 border border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
            Drop tickets here
          </div>
        )}
      </div>
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="shrink-0 flex items-center justify-between px-6 h-14 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-transparent backdrop-blur-xs">
        <h1 className="text-lg font-semibold text-slate-900 dark:text-white">Board</h1>
        <select
          value={selectedProject}
          onChange={(e) => setSelectedProject(e.target.value)}
          className="bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-300 rounded-md border border-slate-200 dark:border-slate-700 px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500 shadow-2xs"
        >
